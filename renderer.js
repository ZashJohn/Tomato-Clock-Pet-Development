...
window.addEventListener("load", () => {
    var isloadednum;
    var firstValue;
    connection.query('SELECT count(*) FROM user WHERE isload = 1;', (error, results, fields) => {
        if (error) {
...
window.addEventListener('message', function (event) {
    ...
    if (event.data.flag === 'nowtasktimeinformation') {
        ...
        connection.query('SELECT total_time FROM user WHERE isload = 1', (error, results, fields) => {
            if (error) {
                console.error('查询失败: ' + error.stack);
                return;
            }
            ...
            //向数据库中添加数据
            connection.query('UPDATE user SET total_time = ? WHERE isload = 1', nowtime, (error, results, fields) => {
                if (error) {
                    console.error('插入数据失败: ' + error.stack);
                    return;
                }
...
window.addEventListener('message', function (event) {
    ...
    if (event.data.flag === 'usersinformation') {
        ...
        for (var i = 0; i < userinf.length; i++) {
            ...
            connection.query('INSERT INTO user (username,email,password) VALUES (?,?,?)', [fullname, email, password], (error, results, fields) => {
                if (error) {
                    console.error('插入数据失败: ' + error.stack);
                    return;
                }
...
window.addEventListener('message', function (event) {
    ...
    if (event.data.flag === 'isloadedinformation') {
        ...
        // 修改isload数据位
        connection.query('UPDATE user SET isload = 1 WHERE email = ?', loadeduseremail, (error, results, fields) => {
            if (error) {
                console.error('插入数据失败: ' + error.stack);
                return;
            }
...
window.addEventListener('message', function (event) {
    ...
    if (event.data.flag === 'loadoutinformation') {
        ...
        // 修改信息位
        connection.query('UPDATE user SET isload = 0 WHERE isload = 1', (error, results, fields) => {
            if (error) {
        ...
        connection.query('TRUNCATE TABLE base.task_name', (error, results, fields) => {
            if (error) {
                console.error('查询失败: ' + error.stack);
                return;
            }
